...
RedTag.addEventListener("click", () => {
  ...
  BorderTag = {
    class:
      "before:absolute before:right-0 before:top-[10%] before:h-[80%] before:w-[4px] before:bg-red-500 before:rounded-l-2xl",
  };
  checkinputs();
});
YellowTag.addEventListener("click", () => {
  ...
  BorderTag = {
    class:
      "before:absolute before:right-0 before:top-[10%] before:h-[80%] before:w-[4px] before:bg-yellow-500 before:rounded-l-2xl",
  };
  checkinputs();
});
GreenTag.addEventListener("click", () => {
  ...
  BorderTag = {
    class:
      "before:absolute before:right-0 before:top-[10%] before:h-[80%] before:w-[4px] before:bg-green-600 before:rounded-l-2xl",
  };
  checkinputs();
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderlist(list) {
    TaskList.innerHTML = list
      .map(
        (task) =>
          `<li data-id=${task.id} class="relative dark:bg-[#091120] bg-white border border-gray-300 rounded-lg shadow-sm">
        ...
        <div class="lg:flex lg:flex-row">
          <div class="flex justify-start items-center gap-5 m-3!">
             ...
             <strong class="text-xl dark:text-white">${task.title}</strong>
          </div>
...
const preventDuplicateObserver = new MutationObserver(() => {
  todayTasksList.querySelectorAll("li").forEach((li) => {
    ...
    const paragraph =
      li.querySelector("p.text-gray-500")?.textContent?.trim() || "";

    if (
      completedTasks.some((t) => t.title === title && t.paragraph === paragraph)
    ) {
      li.remove();
    }
  });
...
preventDuplicateObserver.observe(todayTasksList, {
  childList: true,
  subtree: true,
});
...
document.addEventListener("change", (e) => {
  ...
  const title =
    taskItem.querySelector("strong")?.textContent?.trim() ||
    taskItem.querySelector("p.font-semibold")?.textContent?.trim() ||
    "";

  const paragraph =
    taskItem.querySelector("p.text-gray-500")?.textContent?.trim() || "";

  // پیدا کردن اطلاعات ذخیره‌شده (اگر وجود داشته باشه)
  const savedTask = completedTasks.find(
    (t) => t.title === title && t.paragraph === paragraph
  );
  ...
  if (checkbox.checked && isInToday) {
    ...
    else if (bodertagsClass.includes("yellow"))
      doneBorder = "before:bg-yellow-500";
    else if (bodertagsClass.includes("green"))
      doneBorder = "before:bg-green-500";
    ...
    JavaScript;
  } else if (!checkbox.checked && isInCompleted) {
    // ← برگشت به لیست امروز
    ...
    <li class="relative bg-white border border-gray-300 rounded-lg shadow-sm">
      ...
      <div class="lg:flex lg:flex-row">
        ...
        ${
          tag
            ? `<span class="inline-block mr-10! mb-3! mt-1! px-3 py-1 lg:mr-5! lg:mt-4! max-w-full rounded-lg text-sm ${tag.class}">${tag.text}</span>`
            : ""
        }
      </div>
      ${
        paragraph
          ? `<p class="text-gray-500 m-3! max-w-full text-ellipsis">${paragraph}</p>`
          : ""
      }
    </li>
  `;
